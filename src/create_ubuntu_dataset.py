...
def translate_dialog_to_lists(dialog_filename):
    ...
    # Use standard csv instead of unicodecsv for Python 3 compatibility
...
def prepare_data_maybe_download(directory):
  ...
  # Skip downloading and extraction as the user has already done it
  print("Using existing dialogs in {}".format(dialogs_path))
  return
...
if __name__ == '__main__':
    ...
    def create_eval_dataset(args, file_list_csv):
        ...
        # training dataset
        f = open(os.path.join(os.path.dirname(__file__), "meta", file_list_csv), 'r')
    ...
    def train_cmd(args):
        ...
        f = open(os.path.join(os.path.dirname(__file__), "meta", "trainfiles.csv"), 'r')
        dialog_paths = list(map(lambda path: os.path.join(args.data_root, "dialogs", path), convert_csv_with_dialog_paths(f)))
